...
object VoiceUtil {
    ...
    fun voice2text(context: Context, callback: (String) -> Unit = { println(it) }) {
//        val rec = SpeechRecognizer.createSpeechRecognizer(context, null)
        ...
        dialog.setListener(object : RecognizerDialogListener {
            override fun onResult(recognizerResult: RecognizerResult?, b: Boolean) {
                val result = recognizerResult?.resultString
